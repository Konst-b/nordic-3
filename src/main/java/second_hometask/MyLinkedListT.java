...
public class MyLinkedListT<T> implements Collection <T>, Iterable <T> {
    ...
    final int SIZELIST = 5000000;  //константа для текстирования скорости сортировки
    boolean isThreads = true;      //переменная для тестирования скорости сортировки с потоками и без
    Node <T> head;
    Node <T> last;
    int size;
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < cSort.SIZELIST; i++)
            cSort.add((int) (Math.random() * 1000));
        long en = System.nanoTime();

        System.out.println("Время загрузки списка:" + String.valueOf((en - st) / 100000));
        ...
        en = System.nanoTime();
        System.out.println("Время сортировки списка: " + String.valueOf((en - st) / 100000) +
                "  /элементов:" + cSort.size());
    ...
    public Node <T> sort(Node <T> upper, int sizeL) throws InterruptedException { //возвращает головную ноду после сортировки слиянием
        SortThread t1 = null, t2 = null;
        int upSize = sizeL / 2;
        ...
        if (upSize >= 2)
            if (isThreads) {
                t1 = new SortThread(upper, upSize);
                t1.start();
            } else
                upper = sort(upper, upSize);
        if (lowSize >= 2)
            if (isThreads) {
                t2 = new SortThread(lower, lowSize);
                t2.start();
                isThreads = false;
            } else
                lower = sort(lower, lowSize);
        if (t1 != null) {
            t1.join();
            upper = t1.n;
        }
